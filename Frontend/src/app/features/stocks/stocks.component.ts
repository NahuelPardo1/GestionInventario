...
import { StockDto, StockCreateDto } from '../../core/models/venta.interfaces';
  ...
  template: `
    <div class="h-full flex flex-col gap-6 animate-fade-in">
      <div class="glass-panel p-6 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 border border-white/5 relative overflow-hidden">
        <div class="absolute -right-10 -top-10 w-40 h-40 bg-rose-500/10 rounded-full blur-2xl pointer-events-none"></div>
        <div>
          <h2 class="text-2xl font-bold text-white tracking-tight">Movimientos de Stock</h2>
          <p class="text-slate-400 text-sm mt-1">Historial de entradas y salidas de inventario.</p>
        </div>
      ...
      <div *ngIf="loading" class="flex-1 flex items-center justify-center">
        <div class="text-slate-400 animate-pulse">Cargando movimientos...</div>
      </div>
      ...
      <div *ngIf="!loading" class="glass-panel rounded-2xl border border-white/5 overflow-hidden flex-1">
        <table class="w-full text-left text-sm text-slate-300">
          ...
          <tbody class="divide-y divide-white/5">
            <tr *ngIf="stocks.length===0"><td colspan="5" class="px-6 py-10 text-center text-slate-500">No hay movimientos registrados.</td></tr>
            <tr *ngFor="let s of stocks" class="hover:bg-white/5 transition-colors group">
              <td class="px-6 py-4 font-medium text-white">{{ getLibroTitulo(s.libroId) }}</td>
              <td class="px-6 py-4 text-center">
                <span *ngIf="s.tipo===0" class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">↑ Entrada</span>
                <span *ngIf="s.tipo===1" class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-rose-500/10 text-rose-400 border border-rose-500/20">↓ Salida</span>
              </td>
              <td class="px-6 py-4 text-center font-semibold text-white">{{ s.cantidad }}</td>
              <td class="px-6 py-4 text-slate-400">{{ s.fecha | date:'dd/MM/yyyy HH:mm' }}</td>
              <td class="px-6 py-4 text-center">
                <div class="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button class="btn-icon danger" (click)="deleteStock(s)"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                </div>
              </td>
            </tr>
          </tbody>
    ...
    <div *ngIf="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
      <div class="glass-panel border border-white/10 rounded-2xl w-full max-w-md shadow-2xl animate-fade-in">
        ...
        <form (ngSubmit)="save()" class="p-6 space-y-4">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-2">
              <label class="text-sm font-medium text-slate-300 ml-1">Tipo *</label>
              <select [(ngModel)]="form.tipo" name="tipo" class="input-premium appearance-none bg-slate-800/50" required>
                <option [ngValue]="0">↑ Entrada</option>
                <option [ngValue]="1">↓ Salida</option>
              </select>
            </div>
            <div class="space-y-2">
              <label class="text-sm font-medium text-slate-300 ml-1">Cantidad *</label>
              <input type="number" [(ngModel)]="form.cantidad" name="cantidad" class="input-premium" required min="1">
            </div>
          </div>
          <div class="space-y-2">
            <label class="text-sm font-medium text-slate-300 ml-1">Fecha *</label>
            <input type="date" [(ngModel)]="form.fecha" name="fecha" class="input-premium" required>
          </div>
          <div class="pt-4 flex justify-end gap-3 border-t border-white/5">
            <button type="button" (click)="showModal=false" class="btn-secondary">Cancelar</button>
            <button type="submit" class="btn-primary" [disabled]="saving">{{ saving ? 'Guardando...' : 'Registrar' }}</button>
          </div>
        </form>
      </div>
    </div>
...
export class StocksComponent implements OnInit {
  ...
  form: StockCreateDto = { libroId: 0, tipo: 0, cantidad: 1, fecha: new Date().toISOString().split('T')[0] };
  ...
  load() { this.loading = true; this.stockService.getAll(this.page).subscribe({ next: r => { this.stocks = r.items; this.totalPages = r.totalPages; this.loading = false; }, error: () => this.loading = false }); }
  changePage(p: number) { this.page = p; this.load(); }
  openAdd() { this.form = { libroId: 0, tipo: 0, cantidad: 1, fecha: new Date().toISOString().split('T')[0] }; this.errorMsg = ''; this.showModal = true; }
  save() {
    this.saving = true; this.errorMsg = '';
    this.stockService.create(this.form).subscribe({
      next: () => { this.saving = false; this.showModal = false; this.notify('Movimiento registrado.', 'success'); this.load(); },
      error: (e) => { this.saving = false; this.errorMsg = e.error?.message || e.error?.Message || 'Error al registrar.'; }
    });
  }
  deleteStock(s: StockDto) {
    if (!confirm('¿Eliminar este movimiento?')) return;
    this.stockService.delete(s.id).subscribe({ next: () => { this.notify('Movimiento eliminado.', 'success'); this.load(); }, error: (e) => this.notify(e.error?.message || 'Error.', 'error') });
  }
